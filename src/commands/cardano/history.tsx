...
export function CardanoHistory({ address, network, json, limit, page }: CardanoHistoryProps) {
  ...
  if (loading)
    return (
      <Box>
        <Text>⏳ Fetching transaction history...</Text>
      </Box>
    );
  if (error)
    return (
  ...
  return (
    <Box flexDirection="column" padding={1}>
      ...
      <Box marginTop={1} marginBottom={1}>
        ...
        <Text bold>{result.page}</Text>
        <Text color="gray"> • Showing </Text>
        <Text bold>{result.count}</Text>
        <Text color="gray"> transactions</Text>
        {result.hasMore && <Text color="gray"> • More available →</Text>}
      </Box>
          ...
          {result.transactions.map((tx, i) => (
            ...
            >
              <Box>
                ...
                <Text color="gray">{formatRelativeTime(tx.blockTime)}</Text>
              </Box>
              <Box>
                <Text color="gray">Hash: </Text>
                <Text>{tx.txHash.slice(0, 32)}...</Text>
              </Box>
              <Box>
                <Text color="gray">Block: </Text>
                <Text>{tx.blockHeight.toLocaleString()}</Text>
                <Text color="gray"> • {formatTime(tx.blockTime)}</Text>
              </Box>
              <Box>
                <Text color="gray">Inputs: </Text>
                <Text>{tx.inputCount}</Text>
                <Text color="gray"> → Outputs: </Text>
                <Text>{tx.outputCount}</Text>
              </Box>
              <Box>
                <Text color="gray">Total Output: </Text>
                <Text color="green">{tx.totalOutputAda} ADA</Text>
              </Box>
              <Box>
                <Text color="gray">Fees: </Text>
                <Text color="red">{tx.feesAda} ADA</Text>
              </Box>
            </Box>
          ))}
        </Box>
      ...
      {result.hasMore && (
        <Box marginTop={1}>
          <Text color="gray">Use --page {result.page + 1} to see more transactions</Text>
        </Box>
      )}
    </Box>
  );
