/**
 * EVM Transaction History command
 */
...
export function EVMHistory({ address, network, json, limit }: EVMHistoryProps) {
  ...
  }, [json, loading, error, transactions, address, network, networkConfig]);
  ...
  if (loading) {
    return (
      <Box>
        <Text>Fetching {networkConfig.name} transaction history...</Text>
      </Box>
    );
  }
    ...
    <Box flexDirection="column" padding={1}>
      <Box marginBottom={1}>
        <Text bold color="cyan">
          {networkConfig.name} Transaction History
        </Text>
      ...
      {transactions.length === 0 ? (
        <Box flexDirection="column">
          <Text color="gray">No recent transactions found in the last 100 blocks.</Text>
          <Box marginTop={1}>
            <Text color="gray">View full history at: </Text>
            <Text color="blue">
            ...
            >
              ...
              {tx.amount && (
                <Box>
                  ...
                  <Text color="green">{tx.amount} {networkConfig.symbol}</Text>
                </Box>
              )}
              ...
              {tx.fee && (
                <Box>
                  ...
                  <Text color="red">{tx.fee} {networkConfig.symbol}</Text>
                </Box>
              )}
            </Box>
          ))}
