...
export async function loadWalletWithKeychain(name: string): Promise<string> {
  ...
  try {
    ...
    if (isWalletFileV1(parsed)) {
      ...
      throw new Error(
        'This v1 wallet requires password. Run migration or use --password.'
      );
    }

    throw new Error('Unsupported wallet file format');
  } catch (error) {
    if (error instanceof Error && (error.message.includes('keychain') || error.message.includes('password'))) {
      throw error;
    }
